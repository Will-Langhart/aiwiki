...
INSERT INTO public.categories (slug, name, description, sort_order) VALUES
  ('vector-databases',  'Vector databases',   'Purpose-built databases for vector embeddings and semantic search',          125),
  ('mlops-training',    'MLOps & training',   'Tools for model training, experiment tracking, and ML lifecycle management', 126),
  ('agent-frameworks',  'Agent frameworks',   'Libraries and platforms for building autonomous AI agents and pipelines',    127),
  ('ai-observability',  'AI observability',   'Monitoring, tracing, and evaluation tools for LLM applications',             128)
ON CONFLICT (slug) DO NOTHING;

-- Add enrichment columns to tools
ALTER TABLE public.tools
  ...
  ADD COLUMN IF NOT EXISTS traffic_tier    text
    CHECK (traffic_tier IN ('small','medium','large','xlarge'));

-- Must drop first since return type changes
DROP FUNCTION IF EXISTS public.search_tools(text,text[],text[],text[],boolean,boolean,integer,integer);

-- Update search_tools RPC to surface new columns
